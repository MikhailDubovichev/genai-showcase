...
- [ ] Step 1: Unified ingestion → `chunks.jsonl`. In `apps/cloud-rag/scripts/seed_index.py` ingest both PDFs (prefer `PyMuPDFLoader`, fallback `PyPDFLoader`) and text/markdown (`.txt`, `.md`) from `rag/data/seed`, apply heading-aware → sentence-window for PDFs and sentence-window for text, then write a single `faiss_index/chunks.jsonl` with fields: `id`, `doc_id`, `source_path`, `page`, `heading_path`, `text`, `created_at`, `hash`.
